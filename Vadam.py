...
class Vadam(Optimizer):
    ...
    def get_updates(self, loss, params):
        ...
        for p, g, m, v, vhat in zip(params, grads, ms, vs, vhats):
            m_t = (self.beta_1 * m) + (1. - self.beta_1) * g + ((self.prior_prec * p)/self.train_set_size)
            v_t = (self.beta_2 * v) + (1. - self.beta_2) * K.square(g)
            #m_t = m_t  / (1. - self.beta_1) # bias correction
            #v_t = v_t / (1. - self.beta_2) # bias correction
            p_t = (p + self.epsilon) - lr_t * m_t / (K.sqrt(v_t) + self.prior_prec/self.train_set_size)
